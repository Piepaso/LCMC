...
//    }
	...
	private boolean step() {
		updateCounterInTextArea("Step number: "+ stepNumber+++"   ");
		int bytecode = fetch();
		int v1, v2;
		int address;
		switch (bytecode) {
		...
		if (this.sp <= this.hp) {
			System.out.println("Segmentation fault: sp = " + this.sp + ", hp = " + this.hp);
			this.outputText.append("Segmentation fault: sp = " + this.sp + ", hp = " + this.hp);
			return false;
		}
		return true;
	}
	...
	private void updateCounterInTextArea(String newText) {
		try {
			int lineCount = outputText.getLineCount();
			// Otteniamo l'inizio dell'ultima riga
			int start = outputText.getLineStartOffset(lineCount - 1);
			int end = outputText.getLineEndOffset(lineCount - 1);

			// Sostituiamo il contenuto dell'ultima riga
			outputText.replaceRange(newText, start, end);
